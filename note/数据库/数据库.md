...
<img src="E:\cpp\note\note_picture\image-20210615133056277.png" alt="image-20210615133056277" style="zoom:80%;" />

- 聚簇索引

  叶子页包含了行的全部数据，但是节点页只包含了**索引列**。

  InnoDB将通过**主键**（primary key）聚集数据，如果没有定义主键，InnoDB会选择一个唯一非空列作为主键，若找不到则会隐式定义一个主键来作为聚集索引的列。

  - 优点：
    1. 由于聚集，所以可以把相关数据保存在一起，这样仅需从磁盘读取少量数据页就可以读出目标数据，减少磁盘IO。
    2. 数据访问更快，聚簇索引将数据和索引保存到同一棵B-Tree中，因此索引中获取数据通常比非空索引中查找更快。
  - 缺点：
    1. 聚簇索引最大限度地提高了IO密集型应用的性能，使得速度从随机IO提高到顺序IO，而如果数据都存储到内存中，则访问顺序就没有那么重要了，内存的随机IO与磁盘的顺序IO速度相差无多，而且还需维护索引，因此聚簇索引就没了优势。
    2. 插入速度严重依赖于插入顺序。按照主键的大小顺序插入可以避免频繁的页分裂、页合并，当这一行插入到一个已满的页面时（InnoDB默认最大填充因子是页大小的15/16，留出部分空间用于以后的修改）存储引擎将会分裂两个页面来容纳该行，如果不是按主键顺序加载数据，最好用`optimize table`命令重新组织一下表。
    3. 更新聚簇索引列的代价很高，因为会强制InnoDB将每个被更新的行移动到新的位置，造成页分裂、页合并。
    4. 聚簇索引可能导致全表扫描变慢，尤其是行比较稀疏，或由于页分裂导致数据分布不连续

- 非聚簇索引

  MyISAM的聚簇和非聚簇索引的叶子节点都包含指向该数据行所在物理位置的指针，表则按插入顺序而顺序存储，因此当发生表中顺序变化时，指针的位置总是需要维护、修改。

  InnoDB的非聚簇索引中叶子节点存储索引列和主键列的信息。

  - 缺点：

    1. 二级索引（非聚簇索引）可能比想象的要大，因为二级索引的叶子节点包含了对应行的主键列，因此主键越大二级索引也会越大。

    2. 二级索引访问需要两次索引查找，而不是一次

       对于二级索引的索引列查找到该行的主键，然后再从聚簇索引中找出该主键的数据行。这就是所谓的**回表查询**，先定位主键值，再定位行记录，它的性能较扫一遍索引树更低。

       对于InnoDB的自适应哈希索引能够减少这样重复的工作。

       ![image-20210615133847220](E:\cpp\note\note_picture\image-20210615133847220.png)



#### 覆盖索引

如果一个索引包含所有要查询的字段的指，我们就称为覆盖索引，既然包含所有我们要查询的字段，那么也就不用进行回表查询去获得完整的数据行。

如果索引不能覆盖查询所需的全部列，那就不得不每扫描一条索引记录就都回表查询一次对应的行。

不是所有类型的索引都能成为覆盖索引，因为覆盖索引需要存储列的值，像哈希索引，空间索引和全文索引等都不存储列的原值，所以MySQL只能使用B-Tree索引做覆盖索引。而且也不是所有引擎都能支持覆盖索引。

- 优点
  1. 对于索引条目远小于数据行大小，只读取我们的索引条目的数据将极大减小访问的数据量
  2. 一些存储引擎如MyISAM在内存只缓存索引，若果使用覆盖索引，那么将不用再使用系统调用获取磁盘中完整的数据行
  3. 减少回表查询的开销

当发起索引覆盖的查询时，在Explain的extra可以看到using index

#### 使用索引扫描来排序

索引本身的数据结构就是按索引字段进行排序，如果使用order by时字段顺序能够和索引一致，那么可以利用同一个索引既排序又查询。设计索引时能满足这两个情况是最好的。

order by子句和查询的限制是一样的：需要满足索引的最左前缀的要求

- 有一种情况下可以不满足该条件，只要前导列（where 或 join条件对这些列指明）为常量，就可以弥补该不足。



#### 压缩（前缀）索引

MyISAM压缩每个索引块的方法是，先完全保存索引块中的第一个值，然后将第一个值进行比较得到相同前缀的字节数和剩余的不同后缀部分，把这部分存储起来即可。

- 例如，第一个值“perform”，第二个值“performance"，那么第二个值的前缀压缩后存储为类似”7，ance"这样的形式

因为每个值的压缩前缀都依赖于前面的值，所以MyISAM查找时无法在索引块中使用二分查找而只能从头开始查找。
...
MySQL允许在相同列上创建多个索引，无论有意还是无意，MySQL需要单独维护重复的索引，并且优化器在优化时也许逐个考虑
...
- 重复索引
  ...
  上述为同一列创建了三个索引，即使本意不是这样，但是**MySQL的唯一限制和主键限制都是通过索引来实现的。**

- 冗余索引

  如果创建了索引(A,B)，再创建索引A就是冗余索引，因为索引A可以索引(A,B)的B-Tree使用。但是再创建索引（B,A）或者（B）就不是冗余索引，归根到底是底层B-Tree能不能利用到。

  大多数情况下不需要冗余索引，应该尽量扩展已有的索引而不是创建新的索引。但也有时候处于性能的考虑需要冗余索引，因为扩展已有的索引可能会导致其变得太大，从而影响其它使用该索引的查询性能。

#### 索引和锁

索引可以让查询锁定更少的行，InnoDB只有在访问行时才对其加锁，而索引能够减少InnoDB访问的行数，从而减少锁的数量。

如果索引无法过滤掉无效的行，那么在InnoDB检索到数据并返回给服务器层以后，MySQL服务器才能应用where子句，这时无法避免锁定行了。

- InnoDB在二级索引上使用共享锁，但访问主键使用排他锁。这消除了使用覆盖索引的可能性，并且使select for update比lock in share mode或非锁定查询慢得多。
...
例如网上商城有很多商品类目以及特征，最好的方法是重用索引，而不是为每种组合都去创建索引。

筛选属性（A,B）以及筛选组合（C,A,B）,我们可以创建索引（C,A,B）那我们如何在满足最左前缀匹配原则的情况下索引（A,B)呢？

如果C的**枚举值不多**，我们可以使用 where C in(x,y,z) and A = a and B =b来匹配C，从而重用索引（C,A,B）

- 使用`explain`难以区分MySQL要查询的是范围值还是列表值，因为都是在type列中用range描述。

  对于范围条件查询，MySQL无法再使用范围列后面的索引列了，但是对于“多个等值条件查询”则没有这个限制
...
高性能 -> 查询优化、索引优化、库表结构优化

- 查询为什么会变慢？

  查询的生命周期大致可以按照顺序来看：从客户端，到服务器，然后在服务器上进行解析，生成执行计划，执行，并将结果返回。

  其中执行应作为时间花费的主要环节，包括大量为了检索数据到存储引擎的调用以及调用后的数据处理，包括排序、分组。
...
- 常见的错误
  - 多表关联时返回全部列
  - 重复查询相同的数据
  - 误以为只会返回需要的数据，实际上MySQL返回整个结果集再进行计算
...
- 最简单衡量查询开销的三个指标：
  1. 响应时间：包括真正的执行时间 + 等待时间，这一部分难以测量，只能测得总的响应时间。
  2. 扫描的行数
  3. 返回的行数：最理想情况下扫描的行数和返回的行数应该相同

- EXPALIN语句中的type列反应了访问类型：
...
- **MySQL执行一个查询的过程**
  ...
  ![image-20210621081131869](E:\cpp\note\note_picture\image-20210621081131869.png)
...
MySQL客户端和服务器之间的通信协议是“半双工”的，客户端和服务器之间不同同时收发数据。

因此当传送的数据包太大时，传输占用的时间也会很久。参数`max_allowed_packet`对数据包进行控制。即便如此，该参数对大数据包进行拆包，但接收时也必须接收到所有数据包，而不是收到前几个数据包就可以停止发送，所以查询中的`limit`也很重要。

默认客户端获得全部结果集并缓存到内存中，当多数连接MySQL的库函数从MySQL获取数据时，其结果看起来都像是从MySQL获取数据，而实际上是从这个库函数的缓存中读取。

```
//不同编程语言可能会有差异
link = mysql_connect('lcoalhost','user','p4ssword');
result = mysql_query('select * from table1',&link);
while( row = mysql_fetch_array(&result)){		//看起来像我们需要时才从这个缓存中逐行取出数据
	
}
//如果用下面mysql库函数则不会缓存
result = mysql_unbuffer_query(...)
```
...
- 查询状态
  ...
  查询一个MySQL连接，或者一个线程当前正在做什么，`show full processlist`

  结果中的 Command列就是状态：

  1. Sleep : 线程正在等待客户端发送新的请求
  2. Query：正在执行查询或者正在发送数据给客户端
  3. Locked：在MySQL服务层正在等待锁
  4. Analyzing and statistics：线程正在收集存储引擎的统计信息，并在生成查询的执行计划
  5. Copy to tmp table [on disk]：group by操作，文件排序操作或者union操作，如果有on disk标记，标识MySQL正在将临时内存表放到磁盘上。
  6. Sorting result
  7. Sending data
