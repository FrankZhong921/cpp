...
5. 当master通知reduce worker这些位置时，reduce worker使用远程过程调用读取map worker磁盘上的数据。当读取完所有键值对后，根据中间键进行排序，方便具有相同键的数据进行合并。（之所以需要进行排序，是因为通常有许多不同的键映射到相同的reduce任务。）如果数据量太大，还需要使用外部排序(extetnal sort)
